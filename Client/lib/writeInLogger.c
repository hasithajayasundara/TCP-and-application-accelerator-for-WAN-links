...
void print_tcp_packet(unsigned char* Buffer, int Size, FILE *logfile)
{
    ...
    struct tcphdr *tcph=(struct tcphdr*)(Buffer + iphdrlen + sizeof(struct ethhdr));//set tcp header
    ...
    logHexData(Buffer + header_size , totLength-(iphdrlen + tcphdrlen),logfile );
...
void logRawData (unsigned char* data , int Size,FILE *logfile)
{
	...
	int i , j;
	    for(i=0 ; i < Size ; i++)
	    {
	    	...
	    	/*
	        if( i!=0 && i%50==0)   //if one line of hex printing is complete...
	        {
	            fprintf(logfile , "         ");
	            fprintf(logfile , "\n");
